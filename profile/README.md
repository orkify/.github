# orkify

orkify brings a cloud-like deployment workflow to self-hosted Node.js and Next.js apps.

It is built for developers and teams that need more than a simple Node process manager, without the operational overhead of complex container orchestration. Orkify helps you run apps on your own infrastructure with a more modern, platform-like workflow.

The free and open-source CLI includes:

- process management
- zero-downtime deployments
- log aggregation & rotation
- orchestration across hosts
- basic MCP support for local development and remote apps

The optional SaaS dashboard adds:

- real-time metrics
- error tracking with deobfuscated stack traces
- alerts and notifications
- rolling deployments
- built-in support chat
- Discord and Slack integrations
- team collaboration across multiple hosts
- extended MCP authentication for teams

Runs on Windows, macOS, and Linux, with support for container-based workflows.

- [Website](https://orkify.com)
- [Docs](https://orkify.com/docs)
- [CLI Repository](https://github.com/orkify/orkify)
